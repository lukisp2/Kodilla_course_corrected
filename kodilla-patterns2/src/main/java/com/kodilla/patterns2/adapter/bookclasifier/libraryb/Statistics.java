...
public class Statistics implements BookStatistics {
    ...
    @Override
    public int averagePublicationYear(Map<BookSignature, Book_B> books) {
        if (books.size() == 0) return 0;
        int sum = 0;
        for (Map.Entry<BookSignature, Book_B> entry : books.entrySet()) {
    ...
    @Override
    public int medianPublicationYear(Map<BookSignature, Book_B> books) {
        if (books.size() == 0) return 0;
